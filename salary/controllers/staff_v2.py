...
class StaffView(View):
    def get(self, req, college_id):
        ...
        staffs = []
        college_staffs_qs = college.staffs_v2.filter(status=constants.StaffStatus.ACTIVE).prefetch_related('role_params')
        for staff in college_staffs_qs: # type: Staff_v2
            staffs.append({
                "staff_obj": staff,
...
class AddRoleView(View):
    def get(self, req, staff_id):
        staff: Staff_v2 = Staff_v2.objects.filter(pk=staff_id).select_related('college').first()
        if staff is None:
            raise Http404
...
class Action_AddRole(View, ValidateRoleParamMixin):
    def _clean_input(self, bag):
        staff: Staff_v2 = Staff_v2.objects.filter(pk=helpers.to_int(bag.get('staff_id'))).select_related('college', 'person').first()
        if staff is None:
